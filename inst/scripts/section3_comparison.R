...
n_rep  <- 1000L     # raise to 500 for publication quality; 100 takes ~20-30 min
...
for (s in seq_along(sigmas)) {
  ...
  fits_tv  <- fits_atw <- fits_mle <- fits_ss <- matrix(NA_real_, n_rep, length(x_grid))
  ...
  for (r in seq_len(n_rep)) {
    ...
    # --- MLE ---
    mle <- tryCatch(
      auto_ssj_mle(y, x, ic = "ebic", ebic_xi = 0.1),
      error = function(e) NULL
    )
    if (!is.null(mle)) {
    ...
    # --- Smoothing spline (GCV-selected) ---
    fit_ss <- tryCatch(smooth.spline(x, y), error = function(e) NULL)
    if (!is.null(fit_ss))
      fits_ss[r, ] <- predict(fit_ss, x_grid)$y

    if (r == 1L)
      samp <- list(x = x, y = y, tv = fit_cssd, atw = atw, mle = mle, ss = fit_ss)
  ...
  rmse_ss  <- sqrt(mean((fits_ss  - true_mat)^2, na.rm = TRUE))

  results[[s]] <- list(
    sigma    = sigma,
    samp     = samp,
    rmse     = c(cssd = rmse_tv, atw = rmse_atw, mle = rmse_mle, ss = rmse_ss),
    mean_tv  = colMeans(fits_tv,  na.rm = TRUE),
    mean_atw = colMeans(fits_atw, na.rm = TRUE),
    mean_mle = colMeans(fits_mle, na.rm = TRUE),
    mean_ss  = colMeans(fits_ss,  na.rm = TRUE),
    mse_tv   = colMeans((fits_tv  - true_mat)^2, na.rm = TRUE),
    mse_atw  = colMeans((fits_atw - true_mat)^2, na.rm = TRUE),
    mse_mle  = colMeans((fits_mle - true_mat)^2, na.rm = TRUE),
    mse_ss   = colMeans((fits_ss  - true_mat)^2, na.rm = TRUE),
    ...
    q_ss     = apply(fits_ss,  2L, quantile, c(.025, .975), na.rm = TRUE),
    disc_tv  = disc_tv,
    disc_atw = disc_atw,
    disc_mle = disc_mle
  )
...
# Fitted-curve panel: true signal + MC band + mean estimate + jump marks
...
col_lines <- c("steelblue", "darkgreen", "darkorange3", "purple4")
...
for (m in seq_along(methods)) {
  ...
  pdf(file.path(script_dir, fname), width = 10,
      height = if (has_disc) 6 else 3.5)
  if (has_disc) {
    ...
    op <- par(mfrow = c(1L, 3L), mar = c(3.5, 3.5, 2.5, 0.5),
              mgp = c(2, 0.5, 0), tcl = -0.25, cex = 0.85, cex.main = 0.95)
  }
...
# ====================================================================
# Combined figure: CSSD, ATW, MLE (curve + histogram) + SS (curve only)
# 7 rows x 3 cols: 3 pairs (curve/hist) then 1 curve-only row for SS
# ====================================================================

pdf(file.path(script_dir, "fig_combined.pdf"), width = 10, height = 17)
mat <- matrix(seq_len(21L), nrow = 7L, ncol = 3L, byrow = TRUE)
layout(mat, heights = c(rep(c(3.5, 1.8), 3L), 3.5))
op <- par(mgp = c(2, 0.5, 0), tcl = -0.25, cex = 0.80, cex.main = 0.93)
...
for (m in seq_along(methods)) {
  ...
  par(mar = c(if (has_disc) 1.5 else 3.5, 3.5, 2.5, 0.5))
  for (s in seq_along(sigmas)) {
    draw_fit_panel(results[[s]], method = meth, col_line = col_line,
                   main      = bquote(.(lab) * "  (" * sigma * " = " * .(sigmas[s]) * ")"),
                   ylab      = if (s == 1L) "y" else "",
...
pdf(file.path(script_dir, "fig_bias_mse.pdf"), width = 13, height = 5.5)
layout(matrix(seq_len(8L), nrow = 2L, ncol = 4L, byrow = TRUE))
...
for (m in seq_along(methods)) {
  ...
  abline(v = true_disc, lty = 3L, col = "grey20")
  for (s in seq_along(sigmas)) {
    col_line <- col_lines[s]
    lines(x_grid, bias_list[[m]][[s]], col = col_line,
            lty = 1, lwd = 1.5)
  }
  if (m == 1L)
    legend("bottomright", legend = sig_labs, lty = 1,
           col = col_lines, lwd = 1.5, bty = "n", cex = 0.78)
}
